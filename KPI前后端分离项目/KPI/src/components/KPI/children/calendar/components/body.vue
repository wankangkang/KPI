    ...
    <div class="dates" ref="dates">
      <div class="dates-bg">
        <div class="week-row" v-for="week in currentDates">
          <div class="day-cell" v-for="day in week" style="height: 50px;"
      ...
      <div class="dates-events">
        <div class="events-week" v-for="week in currentDates">
          <div class="events-day" v-for="day in week" track-by="$index" style="height: 50px;"
            :class="{'today' : day.isToday,
              'not-cur-month' : !day.isCurMonth}" @click.stop="dayClick(day.date, day.events, $event)">
            ...
            <div class="event-box" >
              <p class="event-item"  v-for="event in day.events" v-show="event.cellIndex <= eventLimit"
                ...
                @click="eventClick(day.events,$event)">
                {{isBegin(event, day.date, day.weekDay)}}
              </p>
              <p v-if="day.events.length > eventLimit"
                class="event-item" @click.stop="selectThisDay(day, $event)">
                more
              </p>
              <p class="day-number">{{day.monthDay}}</p>
            </div>
        ...
        <div class="more-body">
          <ul class="body-list">
            <li v-for="event in selectDay.events"
              ...
              @click="eventClick(selectDay.events,$event)">
              {{event.title}}
            </li>
          </ul>
        </div>
  ...
  export default {
    ...
    methods : {
      isBegin (event, date, index) {
        ...
        if (index == 0 || st.toDateString() <= date.toDateString() ||　(date>=st && date<=end)) {
          return event.title
        }
        return '　'
      },
      ...
      eventClick(events, jsEvent) {
        
        jsEvent.stopPropagation()
        let pos = this.computePos(jsEvent.target)
        this.$emit('eventclick', events, jsEvent, pos)
      }
    }
  }
</script>
